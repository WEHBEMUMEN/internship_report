...
\begin{equation}
    \int_{\Omega(\bm{\mu})} \rho \bm{w} \cdot \ddot{\bm{u}} \, \mathrm{d}\Omega + \int_{\Omega(\bm{\mu})} \bm{\varepsilon}(\bm{w}) : \mathbb{C} : \bm{\varepsilon}(\bm{u}) \, \mathrm{d}\Omega = \int_{\Omega(\bm{\mu})} \bm{w} \cdot \bm{b} \, \mathrm{d}\Omega + \int_{\Gamma_N(\bm{\mu})} \bm{w} \cdot \bar{\bm{t}} \, \mathrm{d}\Gamma,
    \label{eq:weak_form}
\end{equation}
where $\mathcal{V}$ and $\mathcal{V}_0$ are standard trial and test function spaces satisfying Dirichlet boundary conditions.
