...
\begin{enumerate}
    \item \textbf{Exact geometry mapping on a parameter-independent reference configuration:} By exploiting the natural spline representation of NURBS geometries \cite{piegl1997, cottrell2009}, parameter-dependent physical domains $\Omega(\bm{\mu})$ are pulled back to a fixed reference configuration $\hat{\Omega}$. This preserves consistent control-point indexing and an invariant algebraic degree-of-freedom space across geometric variations without spatial remeshing or re-interpolation.
    \item \textbf{Projection-based IGA-POD formulation:} We construct a low-dimensional reduced subspace from transient dynamic snapshots \cite{berkooz1993, willcox2002, Benner2015} defined over the common reference domain, ensuring consistent projection across the full parameter space \cite{zhu2018}.
    \item \textbf{Structure-preserving hyper-reduction for accelerated operator evaluation:} We formulate Energy-Conserving Sampling and Weighting (ECSW) \cite{Farhat2014, farhat2015, Maierhofer2022} adapted to isogeometric discretizations, resolving the non-affine dependency in the parametric Jacobian evaluations. Both stiffness and mass operators are integrated over a minimal subset of active knot spans, ensuring online simulation complexity is strictly independent of the full-order IGA mesh size while preserving symmetric positive-definiteness and mechanical stability.
    \item \textbf{Comprehensive verification on 2D and 3D benchmarks:} The proposed approach is demonstrated and rigorously evaluated on representative 2D parameterized elastodynamics (with active shape morphing and ECSW) and 3D transient impulse elastodynamics (demonstrating high-order $C^2$ dynamic reduction).
\end{enumerate}
