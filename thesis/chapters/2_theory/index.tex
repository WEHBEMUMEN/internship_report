...
$\boldsymbol{\mu} = (r, x_c)^T \in \mathcal{D} \subset \mathbb{R}^2$ where $r$ represents the depth of the symmetric circular notches cut out from the top and bottom boundaries, and $x_c$ denotes the spatial position (distance from the clamped boundary on the left) of the notch center along the longitudinal axis of the beam. The notch width factor $\sigma$ is kept constant as a configuration parameter ($\sigma = 0.6$ m).
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Schematic of the parameterized notched cantilever beam configuration $\Omega(\boldsymbol{\mu})$ showing Dirichlet boundary support $\Gamma_D(\boldsymbol{\mu})$ and the design parameters $x_c$ (notch center position) and $r$ (notch depth).}
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        $r$                                  & Geometric parameter: depth of the symmetric notches                   \\
...
\end{equation}
Applying Green's divergence theorem over the parameterized spatial domain $\Omega(\boldsymbol{\mu})$ yields the weak form:
\begin{equation}
