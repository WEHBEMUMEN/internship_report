...
\begin{enumerate}
    ...
    \item \textbf{Geometric and Parametric Compatibility}: The reduced basis must accommodate dynamic shape modifications (varying notch positions $\mu$ and radii $r$) without suffering from vector space incompatibility or requiring expensive online re-training.
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!65!black,title={\textbf{Step 1: Offline Snapshot Collection (Data Generation)}}, fonttitle=\bfseries]
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!65!black,title={\textbf{Step 2: Proper Orthogonal Decomposition (Basis Construction)}}, fonttitle=\bfseries]
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!65!black,title={\textbf{Step 3: Pullback Mapping to a Static Reference Domain}}, fonttitle=\bfseries]
...
\begin{equation}
\mat{M}(\boldsymbol{\mu})\ddot{\vect{U}}(t) + \mat{C}(\boldsymbol{\mu})\dot{\vect{U}}(t) + \mathbf{F}_{\text{int}}(\vect{U}(t); \boldsymbol{\mu}) = \mathbf{F}_{\text{ext}}(t; \boldsymbol{\mu})
\end{equation}
...
\begin{itemize}
    ...
    \item \textbf{The Controller}: Event listeners that monitor user inputs (parameter sliders for the notch position $\mu$ and radius $r$), trigger the ROM solver, and update the view dynamically.
\end{itemize}
