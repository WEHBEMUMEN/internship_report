...
In Section 5.1, the high-fidelity spatial displacement vector $\underline{\mathbf{U}}(t) \in \mathbb{R}^N$ is projected onto a low-dimensional subspace spanned by a Reduced Basis matrix $\boldsymbol{\Phi} \in \mathbb{R}^{N \times r}$ ($r \ll N$):
\begin{equation}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}[
        ...
        case/.style={rectangle, draw=orange!70, fill=orange!5, thick, rounded corners, text width=2.6cm, minimum height=3.2cm, align=left, font=\sffamily\tiny},
        ...
        % --- PIPELINE HEADERS ---
        \node [below left=0.6cm and 1.2cm of trigger, font=\sffamily\tiny\bfseries, align=center, draw=blue!40, fill=blue!2] (hdr_naive) {NAIVE PHYSICAL PIPELINE \\ (Spatial Remeshing on $\Omega(\boldsymbol{\mu})$)};
        \node [below right=0.6cm and 1.2cm of trigger, font=\sffamily\tiny\bfseries, align=center, draw=cyan!40, fill=cyan!2] (hdr_mapped) {MAPPED PULLBACK PIPELINE \\ (Reference Assembly on $\hat{\Omega}$)};
        ...
        % --- CASES ---
        % Naive Cases
        \node [case, below left=0.4cm and -1.3cm of hdr_naive] (case1) {\textbf{Case 1: Naive FOM} \\ • Full physical remesh \\ • Assemble full size \\ \ \ $\mathbf{K} \in \mathbb{R}^{N \times N}$ \\ • Slow assembly};
        \node [case, below right=0.4cm and -1.3cm of hdr_naive] (case2) {\textbf{Case 2: Naive ROM} \\ • Mesh warps online \\ • Project basis $\boldsymbol{\Phi}$ \\ • Slow projection: \\ \ \ $\mathbf{K}_r = \boldsymbol{\Phi}^T \mathbf{K} \boldsymbol{\Phi}$};
        
        % Mapped Cases
        \node [case, below left=0.4cm and -1.3cm of hdr_mapped] (case3) {\textbf{Case 3: Mapped ROM} \\ • Flat static mesh \\ • Solve in reduced \\ \ \ subspace size $r \times r$ \\ • Loop scales with \\ \ \ full mesh $E$};
        \node [case, below right=0.4cm and -1.3cm of hdr_mapped] (case4) {\textbf{Case 4: ECSW ROM} \\ • Flat static mesh \\ • Integrate over \\ \ \ sparse set $E^*$ only \\ • Scale weights $w_e$ \\ • Fastest solver};

        \draw [arrow] (hdr_naive.south) -| (case1.north);
        \draw [arrow] (hdr_naive.south) -| (case2.north);
        ...
        % --- OUTPUT ---
        \node [block, below=0.5cm of newmark] (visual) {\textbf{Update State Field \& Visual Layer} \\ Reconstruct field: $\vect{U} \approx \boldsymbol{\Phi} \vect{q}$ (Case 2, 3, 4) \\ Render stress contours and update screen};
