...
Evaluating $k(\vect{u}, \delta\vect{u}; \boldsymbol{\mu})$ on a varying domain $\Omega(\boldsymbol{\mu})$ introduces two critical computational hurdles: physical remeshing destroys snapshot SVD consistency, and online assembly over the full physical mesh causes severe latency.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Vector Space Invariance}: Physical remeshing alters node counts and ordering across parameters, ruining SVD basis extraction. In contrast, reference pullback mapping evaluates all snapshots on an invariant domain $\hat{\Omega}$, guaranteeing SVD vector consistency.
    \item \textbf{Online Assembly Speed}: Physical remeshing requires rebuilding and re-integrating the entire mesh ($\mathcal{O}(E)$ complexity) at every parameter update. Pullback mapping allows precomputing reference terms, reducing online assembly to a minimal active element subset $E^*$.
    \item \textbf{Geometric Precision}: Classical physical remeshing introduces linear boundary tessellation errors, whereas reference pullback mapping preserves the exact mathematical CAD NURBS geometry throughout.
\end{itemize}
