...
Figure \ref{fig:continuum_kinematics} illustrates the kinematic deformation from the initial configuration $\Omega(\boldsymbol{\mu})$ to the current state under external body forces $\vect{f}_b$ and boundary tractions $\vect{h}_N$.
...
To guarantee mathematical consistency and achieve sub-millisecond solves, this thesis selects the **Diffeomorphic Pullback Mapping** approach, shifting parameter dependencies entirely into integration Jacobians on a fixed reference space $\hat{\Omega}$ (detailed in Chapter \ref{ch:pullback}).
