...
\begin{tcolorbox}[colback=lightbg, colframe=primaryblue, title=\textbf{Chapter at a Glance: Real-Time ROM \& Hyper-Reduction}, fonttitle=\bfseries]
    \begin{description}[leftmargin=!, labelwidth=3.4cm]
        ...
        \item[\textbf{Digital Twin}] Pure client-side JavaScript WebGL architecture running sub-millisecond solves ($<1.5\,\text{ms}$) at interactive frame rates ($\ge 24\,\text{FPS}$).
    \end{description}
\end{tcolorbox}

Building upon the high-fidelity FOM baseline results and computational bottleneck identified in Chapter \ref{ch:fom}, evaluating Full Order Models in real time remains computationally prohibitive due to large degrees of freedom ($N = 6,888$) and full-mesh element assembly loops ($E = 3,200$). This chapter formulates the Reduced Order Modeling (ROM) and ECSW hyper-reduction pipeline, projecting state variables onto a low-dimensional POD basis ($r = 15$) and restricting integration to a sparse element subset ($E^* = 28$) to achieve sub-millisecond solve times ($<1.5\,\text{ms}$) for interactive WebGL Digital Twins.

% ------------------------------------------------------------------------------
\section{The Model Reduction Strategy \& 5-Step Recipe}
% ------------------------------------------------------------------------------

The following discussion details the overall 5-step offline/online workflow connecting reference pullback mapping, snapshot POD, ECSW NNLS training, and WebGL execution.
...
\begin{tcolorbox}[colback=lightbg, colframe=primaryblue, title={\textbf{Step 1: Pullback Mapping to Reference Domain $\hat{\Omega}$}}, fonttitle=\bfseries]
...
\newpage
Evaluating singular value energy decay demonstrates rapid spectral convergence, confirming that $r = 15$ modes capture $99.999\%$ of total system energy (Figure \ref{fig:pod_energy_decay}).
...
This rapid singular value energy decay (Figure \ref{fig:pod_energy_decay}) shows that relative projection error drops below $0.11\%$ using only $r=10$ POD modes.
...
\begin{table}[htbp]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{p{7.5cm} p{7.5cm}}
            ...
            \textbf{3. Affine Precomputability}: Reference spatial terms pre-evaluated offline.                                                        &                                                                                                                                                                             \\
            \bottomrule
        \end{tabular}%
    }
\end{table}

Weighing these algorithmic characteristics (Table \ref{tab:mapping_tradeoffs}) confirms that shifting parameter dependency into Jacobians enables offline affine precomputability at the cost of modest push-forward evaluation.
...
The following discussion presents quantitative benchmarks comparing FOM, Galerkin ROM, and ECSW hyper-reduction in terms of solve speed, $L_2$ accuracy, and WebGL frame rates.

Evaluating Model Order Reduction strategies against the high-fidelity Full Order baseline yields key speedup and accuracy trade-offs:
...
These benchmark comparisons (Table \ref{tab:hyper_reduction_metrics}) demonstrate that ECSW hyper-reduction achieves a $64.5\times$ speedup while preserving solution accuracy within $0.054\%$ relative error, enabling real-time client-side WebGL Digital Twin execution at interactive frame rates ($\ge 24\,\text{FPS}$).
...
Overlapping the transient response curves (Figure \ref{fig:ecsw_deflection_plot}) verifies that the hyper-reduced solver reproduces full-order tip deflection $U_y(t)$ with high dynamic fidelity ($0.054\%$ error).
