...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.95, >=stealth]
        ...
        \begin{scope}[xshift=-3.8cm, yshift=0cm, scale=0.75]
            \node[above=0.6cm, font=\small\bfseries\sffamily, text=primaryblue] at (2.5, 1.0) {Physical Domain $\Omega(\boldsymbol{\mu})$};
            % Clamped support
            ...
            \shade[left color=primaryblue!30, right color=accentcyan!20, draw=primaryblue, thick]
            ...
            \node[font=\tiny\bfseries, primaryblue] at (2.6, 0) {$\vect{x} \in \Omega(\boldsymbol{\mu})$};
            \node[font=\tiny, text=black!70] at (2.5, -1.3) {Notched Geometry $\boldsymbol{\mu} = (x_c, r)$};
        \end{scope}

        % CENTER ARROW: Bijective Pullback Diffeomorphism
        \draw[<->, ultra thick, primaryblue] (-0.8, 0) -- (0.8, 0) node[midway, above, font=\small\bfseries] {$\boldsymbol{\Psi}(\hat{\vect{x}}; \boldsymbol{\mu})$} node[midway, below, font=\tiny] {Bijective Pullback};

        % RIGHT PANEL: Static Rectangular Reference Domain \hat{\Omega}
        \begin{scope}[xshift=3.8cm, yshift=0cm, scale=0.75]
            \node[above=0.6cm, font=\small\bfseries\sffamily, text=black!85] at (2.5, 1.0) {Static Reference Domain $\hat{\Omega}$};
            % Clamped support
            \draw[line width=1.5pt] (0,1.0) -- (0,-1.0);
            \foreach \y in {-0.9,-0.6,...,0.9} {
                \draw (0,\y) -- (-0.2,\y-0.12);
            ...
            \node[font=\tiny\bfseries, black!85] at (2.5, 0) {$\hat{\vect{x}} \in \hat{\Omega}$};
            \node[font=\tiny, text=black!70] at (2.5, -1.3) {Invariant Rectangular Grid $[0,L] \times [0,H]$};
        \end{scope}
    \end{tikzpicture}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \begin{tikzpicture}[scale=0.72, every node/.style={transform shape}]
            ...
            \foreach \x in {0,...,3} {
                \foreach \y in {0,...,3} {
                    \draw[fill=primaryblue!20, draw=primaryblue!70] (\x*0.4, \y*0.4) rectangle (\x*0.4+0.35, \y*0.4+0.35);
                }
            }
            \node[below, font=\tiny\bfseries] at (0.75, 0) {Full Mesh ($E = 3,200$ elements)};
            \node[right, font=\tiny, align=left] at (1.65, 0.8) {\textbf{Evaluate ALL}\\\textbf{3,200 elements}};
            
            \draw[->, thick, primaryblue] (0.75, -0.15) -- (0.75, -0.65);
            
            % Giant Matrix N x N
            \draw[fill=gray!15, draw=gray!60, thick] (0.2, -1.7) rectangle (1.3, -0.8) node[midway, font=\scriptsize] {$\hat{\mat{K}}(\boldsymbol{\mu})$};
            \node[below, font=\tiny] at (0.75, -1.7) {$N \times N$ (Full Matrix)};
            
            \draw[->, thick, primaryblue] (1.45, -1.25) -- (1.95, -1.25);
            
            % Small Matrix r x r
            \draw[fill=primaryblue!15, draw=primaryblue, thick] (2.1, -1.5) rectangle (2.6, -1.0) node[midway, font=\tiny] {$\mat{K}_r$};
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \begin{tikzpicture}[scale=0.72, every node/.style={transform shape}]
            ...
            \draw[fill=green!50!black!40, draw=green!60!black] (3*0.4, 1*0.4) rectangle (3*0.4+0.35, 3*0.4+0.35) node[midway, font=\tiny\bfseries, scale=0.8] {$w_3$};
            
            \node[below, font=\tiny\bfseries] at (0.75, 0) {Active Subset ($E^* = 28$ elements)};
            \node[right, font=\tiny, align=left] at (1.65, 0.8) {\textbf{Evaluate ONLY}\\\textbf{28 active elements}};
