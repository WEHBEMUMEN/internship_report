...
This section derives the pulled-back weak variational statements, expressing gradient operators and volume integrals on the invariant reference domain.
...
Substituting these transformation laws into the elastodynamic weak form yields the pulled-back variational statement on the reference domain $\hat{\Omega}$:
...
This section contrasts classical physical remeshing against reference pullback integration, demonstrating how pullback eliminates snapshot vector space incompatibility.

Comparing discretization properties highlights key structural differences between the Naive and Pullback approaches:
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm} p{5.5cm} p{5.5cm}}
        ...
        \midrule
        \textbf{Basis Functions} & Parameter-dependent $\mat{R}(\boldsymbol{\mu})$ & Invariant reference basis $\hat{\mat{R}}$ \\
...
Because reference basis functions $\hat{\mat{R}}$ do not depend on parameter $\boldsymbol{\mu}$ (Table \ref{tab:naive_vs_pullback_algebraic}), all snapshot vectors belong to a single vector space $\mathbb{R}^{N_{dof}}$, making Proper Orthogonal Decomposition (POD) mathematically consistent and exact.

% ------------------------------------------------------------------------------
\section{Chapter Summary}
% ------------------------------------------------------------------------------

This section synthesizes the reference pullback mapping framework and previews the Full Order Model simulation pipeline in Chapter \ref{ch:fom}.
