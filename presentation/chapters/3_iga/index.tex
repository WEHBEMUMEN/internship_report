...
\begin{frame}{Why IGA?}
  \small
  Isogeometric Analysis (IGA) bridges the gap between Computer-Aided Design (CAD) and Finite Element Analysis (FEA):
  
  \begin{itemize}
    \setlength{\itemsep}{0.35cm}
    \item \textbf{Direct CAD Integration:} Eliminates the expensive and time-consuming mesh generation step. The simulation runs directly on the NURBS geometry defined in CAD files (\textbf{`.stp`}, \textbf{`.igs`}).
    \item \textbf{Exact Boundary Representation:} Preserves the exact CAD geometry throughout the analysis, eliminating geometric approximation errors.
    \item \textbf{Higher Continuity ($C^{p-1}$):} B-spline and NURBS basis functions provide high regularity across element boundaries, yielding superior accuracy per degree of freedom compared to classical $C^0$ FEM.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{2. B-Spline Basis}
        \scriptsize
        Polynomial basis functions $N_{i,p}(\xi)$ constructed recursively over $\Xi$ (no weights) via Cox-de Boor recurrence relation.
      \end{block}
      \vspace{0.1cm}
      \begin{block}{4. Control Points}
        ...
        \[
          \vect{x}(\xi) = \sum_i R_{i,p}(\xi)\,\vect{P}_i
        \]
