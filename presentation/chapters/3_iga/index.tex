...
\begin{frame}{Exact CAD Geometry: NURBS Circle}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \small
      Traditional FEA approximates curved boundaries using piecewise linear facets, causing geometric errors.
      
      \begin{block}{Rational Basis and Projective Weights}
        \scriptsize
        By injecting non-unity weights $w_i$, NURBS represents conic sections (circles, ellipses) \textbf{exactly}.
      \end{block}
      \begin{block}{Circle Construction}
        \scriptsize
        An exact circle is parameterized using a quadratic NURBS basis with 9 control points (8 active + 1 closing) forming a square octagon control net. 
      \end{block}
      \scriptsize
      Weights on the diagonal vertices are scaled to $w = \cos(45^\circ) = \frac{\sqrt{2}}{2} \approx 0.707$ to preserve radius.
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \scalebox{0.68}{
