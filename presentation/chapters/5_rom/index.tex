...
\begin{frame}{Client-Side Software Architecture (Digital Twin)}
  ...
  \begin{tikzpicture}[
    box/.style={draw=cnamred, fill=cnamlightbg, thick, rounded corners=6pt,
                minimum width=4.5cm, minimum height=2.6cm, align=center}
  ]
    % Model Box
    \node[box] (model) at (0,0) {
      \textbf{\large Model} \\[0.15cm]
      \textbf{IGA-ROM Core} \\[0.1cm]
      {\footnotesize ECSW solver: $2.2$ ms/step} \\
      {\footnotesize Typed arrays (\texttt{Float64Array})}
    };
    
    % Controller Box
    \node[box] (controller) at (5.5,0) {
      \textbf{\large Controller} \\[0.15cm]
      \textbf{User Interface} \\[0.15cm]
      {\footnotesize Interactive UI sliders} \\
      {\footnotesize Direct parameter bindings}
    };
    
    % View Box
    \node[box] (view) at (11.0,0) {
      \textbf{\large View} \\[0.15cm]
      \textbf{HTML5 Canvas} \\[0.1cm]
      {\footnotesize Von Mises stress heatmap} \\
      {\footnotesize Rendered @ 20 FPS}
    };
    
    % Connections
    \draw[<->, >=stealth, very thick, cnamred] (model) -- (controller);
    \draw[<->, >=stealth, very thick, cnamred] (controller) -- (view);
    
    % Labels/Annotations
    \node[above=0.2cm of model, draw=gray!40, fill=gray!5, rounded corners=3pt, font=\tiny, inner sep=3pt] 
      {JSON ROM data loaded dynamically};
      
    \node[above=0.2cm of controller, font=\tiny\bfseries, color=cnamred] 
