...
\begin{frame}{Project Workflow: Completing the Puzzle}
  ...
  {\small\textbf{The Engineering Journey}: Connecting theory, literature, and modern tools into an interactive solver}
  \par\vspace{0.25cm}
  ...
  \begin{tikzpicture}[
    ...
    % Stage 1: Understanding & Scoping
    \node [stage] (s1) at (0, 0) {
      \textbf{\normalsize 1. Scope}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize Problem Insight}\\[2pt]
      {\tiny
      $\bullet$ Initial reading on elastodynamics\\
      $\bullet$ Identified browser real-time bottleneck}
    };

    % Stage 2: Roadmap
    \node [stage] (s2) at (2.85, 0) {
      \textbf{\normalsize 2. Roadmap}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize Strategic Plan}\\[2pt]
      {\tiny
      $\bullet$ Outlined step-by-step milestones\\
      $\bullet$ IGA $\to$ Pullback $\to$ POD $\to$ ECSW $\to$ Web}
    };

    % Stage 3: Prior Literature
    \node [stage] (s3) at (5.70, 0) {
      \textbf{\normalsize 3. Literature}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize Key References}\\[2pt]
      {\tiny
      $\bullet$ Explored state of art\\
      $\bullet$ Hughes/Cottrell (IGA)\\
      $\bullet$ Quarteroni/Benner (ROM)\\
      $\bullet$ Farhat (ECSW)}
    };

    % Stage 4: AI-Assisted Implementation
    \node [stage] (s4) at (8.55, 0) {
      \textbf{\normalsize 4. Coding}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize AI-Assisted Dev}\\[2pt]
      {\tiny
      $\bullet$ AI pairing for complex numerical math\\
      $\bullet$ Rapid prototyping \& JS/WebGL pipelines}
    };

    % Stage 5: Completing the Puzzle
    \node [stage, draw=green!60!black, fill=green!5] (s5) at (11.40, 0) {
      \textbf{\normalsize 5. Deliver}\\[2pt]
      \color{green!60!black}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize The Final Puzzle}\\[2pt]
      {\tiny
      $\bullet$ Unified physics, reduction \& frontend\\
      $\bullet$ Real-time web solver ($< 1.5\,\text{ms}$, $>30$ FPS)}
    };
  ...
  \begin{alertblock}{Completing the Puzzle}
    \centering\footnotesize
    Combining foundational literature, rigorous mathematical reduction, and AI-accelerated implementation to deliver a client-side real-time interactive solver from scratch.
  \end{alertblock}
\end{frame}
