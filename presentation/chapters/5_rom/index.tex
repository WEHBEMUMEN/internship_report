...
\begin{frame}{Project Workflow: Completing the Puzzle}
  ...
  {\small\textbf{The Roadmap to Victory}: Connecting top theory, smart reduction, and modern tools}
  \par\vspace{0.25cm}
  ...
  \begin{tikzpicture}[
    ...
    \node [stage] (s1) at (0, 0) {
      ...
      \textbf{\scriptsize Too Slow!}\\[2pt]
      {\tiny
      $\bullet$ Standard FEM: 142\,ms\\
      $\bullet$ Remeshing is a disaster\\
      $\bullet$ Web needs $<16\,\text{ms}$}
    };

    % Stage 2: The Plan
    \node [stage] (s2) at (2.85, 0) {
      \textbf{\normalsize 2. Strategy}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize Big Blueprint}\\[2pt]
      {\tiny
      $\bullet$ Clear plan from day 1\\
      $\bullet$ Pullback $\to$ Fixed grid\\
      $\bullet$ POD $\to$ ECSW speed}
    };

    % Stage 3: Top Giants
    \node [stage] (s3) at (5.70, 0) {
      \textbf{\normalsize 3. Giants}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize Best Minds}\\[2pt]
      {\tiny
      $\bullet$ Hughes (Exact IGA)\\
      $\bullet$ Quarteroni (Top ROM)\\
      $\bullet$ Farhat (Fast ECSW)}
    };

    % Stage 4: Fast Build
    \node [stage] (s4) at (8.55, 0) {
      \textbf{\normalsize 4. Build}\\[2pt]
      \color{cnamred!70}\hrule\vspace{3pt}\color{black}
      \textbf{\scriptsize AI-Assisted}\\[2pt]
      {\tiny
      $\bullet$ AI pairing for heavy math\\
      $\bullet$ Rapid prototyping\\
      $\bullet$ Clean WebGL engine}
    };
    ...
    \node [stage, draw=green!60!black, fill=green!5] (s5) at (11.40, 0) {
      ...
      \textbf{\scriptsize Total Victory}\\[2pt]
      {\tiny
      $\bullet$ $64.5\times$ faster ($1.3\,\text{ms}$!)\\
      $\bullet$ Rock-solid accuracy\\
      $\bullet$ Smooth 60 FPS in browser}
    };
  ...
  \begin{alertblock}{The Bottom Line}
    \centering\footnotesize
    We took the best theory, crushed the computational bottleneck, and delivered a lightning-fast simulation directly in the browser. Zero lag, pure performance.
  \end{alertblock}
\end{frame}
