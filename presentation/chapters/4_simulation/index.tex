...
\begin{frame}{Simulation Path \& Solver Workflow}
  ...
  \begin{tikzpicture}[
      node distance=1.3cm,
      block/.style={rectangle, draw=blue!70, fill=blue!5, thick, rounded corners, minimum width=6.2cm, minimum height=0.7cm, align=center, font=\sffamily\scriptsize},
      process/.style={rectangle, draw=orange!70, fill=orange!5, thick, minimum width=6.2cm, minimum height=0.7cm, align=center, font=\sffamily\scriptsize},
      math/.style={rectangle, draw=purple!70, fill=purple!5, thick, rounded corners, minimum width=6.2cm, minimum height=0.7cm, align=center, font=\sffamily\scriptsize},
      arrow/.style={thick,->,>=stealth}
  ]
      % Left Column: Classical Physical-Domain
      \node [block] (naive_start) {\textbf{Classical Physical Path} \\ Physical Domain $\Omega(\boldsymbol{\mu})$};
      \node [process, below of=naive_start] (remesh) {Regenerate Mesh \\ Number of DOFs $N(\boldsymbol{\mu})$ varies};
      \node [process, below of=remesh] (assemble) {Assemble $\mat{K}(\boldsymbol{\mu})$ and $\mat{M}(\boldsymbol{\mu})$ \\ High Cost, Online};
      \node [math, below of=assemble] (solve_naive) {Implicit Newmark Step \\ $\mat{K}_{\text{eff}} \vect{U}_{n+1} = \vect{F}_{\text{eff}}^{n+1}$};
      \node [process, below of=solve_naive] (output_naive) {Tip deflection $U_y(t)$ \& $\sigma_{\text{VM}}$ \\ Variable Dimensions};
      
      % Right Column: Mapped Reference
      \node [block, right=1.6cm of naive_start] (mapped_start) {\textbf{Reference Map Path} \\ Fixed Reference Domain $\hat{\Omega}$};
      \node [process, below of=mapped_start] (static_mesh) {Static Reference Mesh \\ Constant DOFs $N$ (no remeshing)};
      \node [process, below of=static_mesh] (pullback) {Pullback Integrals \\ Coordinate Jacobian $\mat{J}(\boldsymbol{\mu})$};
      \node [process, below of=pullback] (pullback_assemble) {Assemble Stiffness $\mat{K}(\boldsymbol{\mu})$ \\ Online Integrals (still costly)};
      \node [math, below of=pullback_assemble] (solve_mapped) {Apply Penalty Clamping BCs \\ $\mat{K}_{\text{eff}}[i,i] \gets k_p = 10^{30}$, $\vect{F}_{\text{eff}}[i] \gets 0$};
      \node [process, below of=solve_mapped] (output_mapped) {Extract QoI ($U_y(t)$, $\sigma_{\text{VM}}$) \\ $T=10\text{ s}$, $200$ time steps ($\Delta t = 0.05\text{ s}$)};
      
      % Connections
      \draw [arrow] (naive_start) -- (remesh);
      \draw [arrow] (remesh) -- (assemble);
      \draw [arrow] (assemble) -- (solve_naive);
      \draw [arrow] (solve_naive) -- (output_naive);
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Computational Bottleneck}:
          \begin{itemize}
            \item Performing the high-fidelity Full Order Model (FOM) simulations online remains computationally expensive.
            \item This offline/online CPU overhead motivates the use of projection-based Reduced Order Models (ROM).
          \end{itemize}
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
