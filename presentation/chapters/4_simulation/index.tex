...
% Slide: Full Order Model (FOM) & Dynamic Solver
\begin{frame}{Full Order Model (FOM) \& Dynamic Solver}
  \begin{block}{Semi-Discrete System of Equations}
    \vspace{-0.1cm}
    \[
      \mat{M}(\boldsymbol{\mu})\,\ddot{\vect{U}}(t) + \mat{C}(\boldsymbol{\mu})\,\dot{\vect{U}}(t) + \mat{K}(\boldsymbol{\mu})\,\vect{U}(t) = \vect{F}_{\text{ext}}(t)
    \]
    \vspace{-0.25cm}
    {\centering\scriptsize Initial Conditions: $\vect{U}(0) = \vect{U}_0, \quad \dot{\vect{U}}(0) = \vect{V}_0$\par}
  \end{block}
  ...
  \begin{itemize}
    ...
    \small
    \item \textbf{Implicit Solver}: Newmark-$\beta$ method ($\beta=0.25, \gamma=0.5$), unconditionally stable.
    \item \textbf{System Size}: $80 \times 40$ elements ($6,888$ DOFs).
    \item \textbf{The Bottleneck}: Re-assembling $\mat{K}(\boldsymbol{\mu})$ and $\mat{M}(\boldsymbol{\mu})$ when $\boldsymbol{\mu}$ changes is too slow for real-time interactivity.
  \end{itemize}
\end{frame}
